...
def generate_summary(root_dir, new_dialogue_number, new_figures_dir):
    ...
    dialogue_figures_dir = os.path.join(new_figures_dir, f"dialogue{new_dialogue_number}")
    if os.path.exists(dialogue_figures_dir):
        print(f"Error Figures directory dialogue{dialogue_figures_dir} already exists!!!")
        return
    else:
        os.makedirs(dialogue_figures_dir, exist_ok=True)
    ...
    for folder in sorted_folders:
        ...
        if os.path.isdir(folder_path):
            ...
            if os.path.exists(json_path):
                ...
                plot_dest_path = os.path.join(dialogue_figures_dir, new_plot_filename)
...
# Usage example
root_dir = "/hri/storage/user/emenende/interaction_recordings/dialogue43"  # Replace with the path to your folders
new_dialogue_number = 7
new_dir = "/hri/storage/user/emenende/myNotes"
new_figures_dir = os.path.join(new_dir, "Figures")
summary = generate_summary(root_dir, new_dialogue_number, new_figures_dir)
if summary:
    # Save summary to a Markdown file
    with open(os.path.join(new_dir,"interaction_summary.md"), "w") as f:
        f.write(summary)